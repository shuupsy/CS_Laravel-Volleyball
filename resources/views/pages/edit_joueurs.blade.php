...
@section('content')
    <section class='flex justify-center'>
        <div
            class='w-3/4 bg-[#f8f7ff]/[.36] border-2 border-slate-500 rounded-lg shadow-lg p-6 flex flex-col items-center justify-center gap-10'>
            ...
            <form action="{{ url('/joueurs' . '/' . $joueur->id) }}" method='post' enctype="multipart/form-data"
                id='edit_player_form' class='flex flex-col gap-3'>
                @csrf
                @method('PATCH')
                ...
                <div class='flex items-center gap-3.5'>
                    ...
                    <input type="text" name="nom" id="nom" value='{{ $joueur->nom_joueur }}'
                        ...
                        class='mb-2 p-0.5 border-b-2 border-[#ffd8be] focus:outline-none focus:border-[#ffb383] ease-in duration-300'>
                </div>
                    ...
                    @foreach ($roles as $role)
                        <label>
                            <input class='hidden' type="radio" name="role" value="{{ $role->id }}"
                                {{ $role->role == $joueur->role->role ? 'checked="checked"' : '' }}>
                ...
                <div class='flex justify-center items-center gap-5'>
                    <button type='submit' class='bg-[#b8b8ff] hover:bg-[#9381ff] text-white rounded-lg px-4 py-2'>METTRE A
                        JOUR</button>
                    <a href="/joueurs">
                        <p
                            class='cursor-pointer text-slate-600 hover:border-[1px] hover:rounded-lg hover:border-slate-600 px-4 py-2'>
                            Cancel</p>
                    </a>
                </div>
